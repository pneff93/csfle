# 🔐 my-platform-gcp-java-client

Platform/GCP Java demo client used in snapshot tests.
...
1. Compiles the project (if needed) and runs `BootstrapTopic` to create the Kafka topic `my-platform-gcp-java-client-0011` (idempotent).
2. Registers the schema for `my-platform-gcp-java-client-0011-value` with the `birthday` field tagged as `PII`.
