# 🔐 my-cloud-gcp-go-client

Cloud/GCP Go demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-cloud-gcp-go-client-0039` (idempotent — safe to re-run).
2. Registers the schema for `my-cloud-gcp-go-client-0039-value` with the `birthday` field tagged as `PII`.
