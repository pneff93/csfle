# 🔐 my-cloud-hashicorp-java-client

Cloud/HashiCorp Vault Java demo client used in snapshot tests.
...
1. Compiles the project (if needed) and runs `BootstrapTopic` to create the Kafka topic `my-cloud-hashicorp-java-client-0016` (idempotent).
2. Registers the schema for `my-cloud-hashicorp-java-client-0016-value` with the `birthday` field tagged as `PII`.
