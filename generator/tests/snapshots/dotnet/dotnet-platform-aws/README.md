# 🔐 my-platform-aws-dotnet-client

Platform/AWS .NET demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-aws-dotnet-client-0025` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-aws-dotnet-client-0025-value` with the `birthday` field tagged as `PII`.
