# 🔐 my-platform-azure-js-client

Platform/Azure JavaScript demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-azure-js-client-0018` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-azure-js-client-0018-value` with the `birthday` field tagged as `PII`.
