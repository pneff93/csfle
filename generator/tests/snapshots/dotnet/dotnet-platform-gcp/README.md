# 🔐 my-platform-gcp-dotnet-client

Platform/GCP .NET demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-gcp-dotnet-client-0027` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-gcp-dotnet-client-0027-value` with the `birthday` field tagged as `PII`.
