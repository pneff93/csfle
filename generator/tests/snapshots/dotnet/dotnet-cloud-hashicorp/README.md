# 🔐 my-cloud-hashicorp-dotnet-client

Cloud/HashiCorp Vault .NET demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-cloud-hashicorp-dotnet-client-0032` (idempotent — safe to re-run).
2. Registers the schema for `my-cloud-hashicorp-dotnet-client-0032-value` with the `birthday` field tagged as `PII`.
