# 🔐 my-platform-hashicorp-java-client

Platform/HashiCorp Vault Java demo client used in snapshot tests.
...
1. Compiles the project (if needed) and runs `BootstrapTopic` to create the Kafka topic `my-platform-hashicorp-java-client-0012` (idempotent).
2. Registers the schema for `my-platform-hashicorp-java-client-0012-value` with the `birthday` field tagged as `PII`.
