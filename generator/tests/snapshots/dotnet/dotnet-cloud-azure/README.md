# 🔐 my-cloud-azure-dotnet-client

Cloud/Azure .NET demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-cloud-azure-dotnet-client-0030` (idempotent — safe to re-run).
2. Registers the schema for `my-cloud-azure-dotnet-client-0030-value` with the `birthday` field tagged as `PII`.
