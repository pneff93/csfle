# 🔐 my-platform-aws-go-client

Platform/AWS Go demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-aws-go-client-0033` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-aws-go-client-0033-value` with the `birthday` field tagged as `PII`.
