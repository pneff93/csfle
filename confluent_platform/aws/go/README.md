# 🔐 Client-Side Field Level Encryption (CSFLE) with Confluent Platform and AWS KMS - Go Client
...
This pulls in `confluent-kafka-go/v2` (Kafka client + Schema Registry + AWS KMS encryption driver) and `godotenv`.
...
This client reads from the **same `.env` file** used by the Python and Java clients, located in the parent directory
(`confluent_platform/aws/.env`). If you haven't created it yet:
...
> 💡 **Why this works:** `godotenv` does not override variables already set in the shell, so the `export`s above take
...
# Or re-load environment variables from parent .env to restore correct credentials
set -a
source ../.env
set +a
```
...
├── avro/
│   └── personal_data.avsc              # Avro schema (reference, registered via curl above)
├── internal/
│   ├── config/config.go                # loads ../.env via godotenv
│   └── model/personal_data.go          # struct with avro:"..." tags
└── cmd/
    ├── producer/main.go                # produces 20 encrypted records
    └── consumer/main.go                # decrypts and prints records
```

The Go struct in `internal/model/personal_data.go` carries `avro:"..."` tags that map to the schema's field names —
this is how the `confluent-kafka-go` Avro v2 serializer knows which struct field maps to which Avro field. The wire-format
schema itself is fetched from Schema Registry at runtime (because we set `AutoRegisterSchemas=false` and
`UseLatestVersion=true`), so the `.avsc` file is included only as a reference for the curl registration command.
