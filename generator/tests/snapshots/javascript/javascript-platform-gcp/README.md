# 🔐 my-platform-gcp-js-client

Platform/GCP JavaScript demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-gcp-js-client-0019` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-gcp-js-client-0019-value` with the `birthday` field tagged as `PII`.
