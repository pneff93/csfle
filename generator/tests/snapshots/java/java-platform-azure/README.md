# 🔐 my-platform-azure-java-client

Platform/Azure Java demo client used in snapshot tests.
...
1. Compiles the project (if needed) and runs `BootstrapTopic` to create the Kafka topic `my-platform-azure-java-client-0010` (idempotent).
2. Registers the schema for `my-platform-azure-java-client-0010-value` with the `birthday` field tagged as `PII`.
