# 🔐 my-platform-azure-go-client

Platform/Azure Go demo client used in snapshot tests.
...
This downloads `confluent-kafka-go/v2` (Kafka client + Schema Registry + Azure Key Vault encryption driver) and `godotenv`, and populates `go.sum`. The `// indirect` block in `go.mod` is also filled in based on the resolved transitive deps for your platform.
...
1. Creates the Kafka topic `my-platform-azure-go-client-0034` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-azure-go-client-0034-value` with the `birthday` field tagged as `PII`.
