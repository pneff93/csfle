# 🔐 my-platform-aws-java-client

Platform/AWS Java demo client used in snapshot tests.
...
1. Compiles the project (if needed) and runs `BootstrapTopic` to create the Kafka topic `my-platform-aws-java-client-0009` (idempotent).
2. Registers the schema for `my-platform-aws-java-client-0009-value` with the `birthday` field tagged as `PII`.
