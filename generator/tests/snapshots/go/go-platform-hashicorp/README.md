# 🔐 my-platform-hashicorp-go-client

Platform/HashiCorp Vault Go demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-platform-hashicorp-go-client-0036` (idempotent — safe to re-run).
2. Registers the schema for `my-platform-hashicorp-go-client-0036-value` with the `birthday` field tagged as `PII`.
