# 🔐 my-cloud-gcp-dotnet-client

Cloud/GCP .NET demo client used in snapshot tests.
...
1. Creates the Kafka topic `my-cloud-gcp-dotnet-client-0031` (idempotent — safe to re-run).
2. Registers the schema for `my-cloud-gcp-dotnet-client-0031-value` with the `birthday` field tagged as `PII`.
